...
class UltraSoundDataSet2(Dataset):
    ...
    def __getitem__(self,idx):
        ...
        label_path = os.path.join(self.root_dir+'/label/','label'+self.sample_list[idx][3:])
...
class UltraSoundDataSet_OF(Dataset):
    ...
    def __getitem__(self,idx):
        ...
        curr_image_path = os.path.join(self.root_dir,'img',self.sample_list[idx])
        curr_label_path = os.path.join(self.root_dir,'label',"label%04d.png" %ser_num)
        image_curr = Image.open(curr_image_path)
        label_curr = Image.open(curr_label_path).convert('L')

        #read previous image-label pair 
        prev_image_path = os.path.join(self.root_dir,'img',"img%04d.png" %(ser_num-1))
        prev_label_path = os.path.join(self.root_dir,'label',"label%04d.png" %(ser_num-1))
