...
void ConfidentMapConvex::saveImageToDisc(MemImage *memImage)
{
    ...
    cv::imwrite("/home/zhongliang/Project/Pro_OrientationAdj/test_saved.jpeg", cv_imgIn);   //good, the croped one
...
//    confidenceAlg->configure();
    ...
    //setting parameters
    properties->setParam("Working Resolution", 7);
...
void ConfidentMapConvex::calFigureDerivation(cv::Mat &cvImage)
{
    ...
    double threshold = 80;
    cv::threshold(dst, thresholdedGradient, threshold, 255, CV_THRESH_BINARY);
    ...
    cv::findContours(thresholdedGradient, contours, hierarchy, CV_RETR_EXTERNAL, CV_CHAIN_APPROX_NONE, cv::Point(0, 0) );
    ...
    for(int i = 0; i < contours.size(); i++)
    {
        ...
        if(size > sizeThreshold)    {
            //delete stuff
            qDebug()<<"size:"<<size;
            cv::drawContours(cvImageBlack, contours, i, color, CV_FILLED);
        }
    }
